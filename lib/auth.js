...
export async function getCurrentUser() {
  ...
  if (user.role === "buyer") {
    user.walletAddress = getDefaultWalletAddressForRole(user.role);
  }
...
export async function registerUser({ name, email, password, role }) {
  ...
  const result = await execute(
    `
      ...
      VALUES (?, ?, ?, ?, '', ?)
    `,
    [name.trim(), normalizedEmail, passwordHash, normalizedRole, getDefaultWalletAddressForRole(normalizedRole)]
  );
